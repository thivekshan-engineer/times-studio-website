...
router.post('/', async (req, res) => {
  try {
    ...
    if (!service || service.trim().length === 0) {
      return res.status(400).json({ success: false, message: 'Service selection is required' });
    }
    ...
    try {
      ...
      await resend.emails.send({
        from: 'Times Studio <onboarding@resend.dev>',
        to: process.env.OWNER_EMAIL,
        subject: `📸 New Booking - ${service} from ${firstName}`,
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 8px;">
            <h2 style="color: #0a1628; border-bottom: 2px solid #f59e0b; padding-bottom: 10px;">📸 New Booking Received!</h2>
      ...
      await resend.emails.send({
        ...
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 8px;">
            <h2 style="color: #0a1628; border-bottom: 2px solid #f59e0b; padding-bottom: 10px;">✅ Booking Received!</h2>
            <p style="color: #334155;">Dear <strong>${firstName}</strong>,</p>
            <p style="color: #334155;">Thank you for contacting <strong>Times Studio & T Marin Air Travels</strong>! We will get back to you shortly.</p>
            <table style="width: 100%; border-collapse: collapse; margin-top: 20px;">
              <tr style="background: #f8fafc;">
